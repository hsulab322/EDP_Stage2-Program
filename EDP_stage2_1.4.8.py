...
# ------Prepare to start the Trial-------
    ...
    # ------Prepare to start Routine "Interval 2s"-------
        continueRoutine = True
    ...
    # -------Ending Routine "2s Interval"-------
        for thisComponent in IntervalComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
                thisComponent.tStopRefresh = tThisFlipGlobal
        # Save the data
        thisExp.addData('2s_interval.started' + '_' + str(r+1) + "-" + str(Trial_n+1), cross.tStartRefresh)
        thisExp.addData('2s_interval.stopped' + '_' + str(r+1) + "-" + str(Trial_n+1), cross.tStopRefresh)
        stage2df = stage2df.append({'Round': str(r+1),
                                    'Trial': '2s_interval_'+str(Trial_n+1),
...
for thisComponent in IncentiveComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
        thisComponent.tStopRefresh = tThisFlipGlobal
thisExp.addData('Incentive.started' + '_' + str(r+1) + "-" + str(Trial_n+1), text_totalIncentive.tStartRefresh)
thisExp.addData('Incentive.stopped' + '_' + str(r+1) + "-" + str(Trial_n+1), text_totalIncentive.tStopRefresh)
...
# keep track of which components have finished
endExpComponents = [OK_endExp,mouse_endExp,text_endExp,key_endExp]
